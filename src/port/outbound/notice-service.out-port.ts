...
export abstract class NoticeServiceOutPort {
  abstract findAll(searchNotice: SearchNoticeDto, paginationQuery: PaginationQuery, status?: string): Promise<[Notice[], number]>;

  abstract findPopupNotices(): Promise<Notice[]>;

  abstract findByNoticeId(id: number): Promise<Notice | null>;

  abstract save(notice: Notice): Promise<Notice>;
